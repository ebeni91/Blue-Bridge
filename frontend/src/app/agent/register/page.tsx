...
export default function AgentDashboard() {
  ...
  // Updated form state to match the new database architecture
  const [formData, setFormData] = useState({
    input_full_name: '', phone_number: '', region: '', address: '', 
    primary_product: '', secondary_product: '', harvest_season: ''
  });
          ...
          {activeTab === 'overview' && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-5xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <StatCard title="Average Trust Score" value="5.0" icon={Award} color="text-amber-600" bg="bg-amber-100" />
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {farmers.map(farmer => (
                    <div key={farmer.id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                      ...
                      <h3 className="text-xl font-bold text-gray-900 mb-1">{farmer.full_name}</h3>
                      ...
                      <div className="pt-4 border-t border-gray-50 flex flex-wrap gap-2">
                        ...
                        {farmer.secondary_product && (
                          <span className="px-3 py-1.5 bg-gray-50 text-gray-600 text-xs font-bold rounded-lg">{farmer.secondary_product}</span>
                        )}
                      </div>
                    </div>
                  ))}
              ...
              <div className="bg-white rounded-3xl border border-gray-100 shadow-xl overflow-hidden">
                ...
                <form onSubmit={handleRegister} className="p-8 space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">Harvest Season</label>
                      <input type="text" required placeholder="e.g. Meher" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:bg-white transition-all font-medium" value={formData.harvest_season} onChange={(e) => setFormData({...formData, harvest_season: e.target.value})} />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">Region</label>
                      <input type="text" required placeholder="e.g. Oromia" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:bg-white transition-all font-medium" value={formData.region} onChange={(e) => setFormData({...formData, region: e.target.value})} />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">Specific Address</label>
                      <input type="text" placeholder="e.g. East Shewa, Lome Woreda" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:bg-white transition-all font-medium" value={formData.address} onChange={(e) => setFormData({...formData, address: e.target.value})} />
                    </div>
                    <div>
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">Secondary Crops</label>
                      <input type="text" placeholder="e.g. Wheat, Maize (Optional)" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:bg-white transition-all font-medium" value={formData.secondary_product} onChange={(e) => setFormData({...formData, secondary_product: e.target.value})} />
                    </div>
                  </div>
                  ...
                  <div className="pt-6 border-t border-gray-100">
                    <button type="submit" className="w-full py-4 bg-green-600 text-white font-extrabold text-lg rounded-xl hover:bg-green-700 shadow-md hover:shadow-lg transition-all active:scale-95 flex justify-center items-center">
                      <UserPlus className="h-5 w-5 mr-2" /> Register & Generate ID
                    </button>
